...
function loadErrorMessage(error: unknown, detail = false): string {
  const fallback = `CS대응 ${detail ? '상세를' : '목록을'} 불러오지 못했습니다.`
  if (!isAxiosError(error)) return fallback
  if (error.response?.status === 401) return '로그인이 만료되었습니다. 다시 로그인해 주세요.'
  if (error.response?.status === 403) return 'CS대응을 조회할 권한이 없습니다.'
  if (error.response?.status === 404) return 'CS대응을 찾을 수 없습니다.'
