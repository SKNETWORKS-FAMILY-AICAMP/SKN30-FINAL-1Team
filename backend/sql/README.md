...
- `20260915_0039_company_scope_schedule_suggestion.sql`: 일정추천을 고객사 단위로 통일하면서
  ...
  개발 DB(session pooler)에 적용했고, 딜 단위 대기 카드 30건이 `expired`로 바뀌었습니다
  (수락 58건·거절 28건·고객사 카드는 그대로). 운영 DB 반영은 배포 절차에서 별도로 적용해야 합니다.
...
- `20260915_0035_customer_company_soft_delete.sql`: `customer_company`에 `deleted_at`을 더해
  ...
  계속 검색되던 문제를 막습니다. 딜·발주·보고서·CS가 이 표를 참조하므로 행은 남기고, id로
  읽는 경로(`GET /customer-companies/{id}`)는 거르지 않아 지난 기록에는 회사 이름이 그대로
  남습니다. 파일 끝의 UPDATE는 고객이 있었다가 모두 지워진 회사만 감춥니다. 고객이 한 번도
  없던 회사는 앱도 감추지 않으므로 함께 건드리지 않습니다. 반복 실행해도 결과가 같습니다.
  2026-09-15 현재 연결된 개발 DB(transaction pooler)에 적용했고, 회사 3932곳 중 3곳
  (`하이픈검증회사`, `서림메디컬센터`, `E2E 레이저 테스트 병원`)이 감춰졌습니다. 고객이 한 번도
...
- `20260915_0037_report_context_rag.sql`: 모든 보고서 제출본을 `report_context_chunk`에
  ...
  원격 DB에는 아직 적용하지 않았습니다. 기존 문맥의 의미검색 벡터는 적용 대상 팀별로
  `uv run python -m scripts.backfill_report_context_vectors --team-id <TEAM_UUID> --apply`를
  실행해 채우며, 실행 전에는 키워드 RAG로 동작합니다.
...
- `20260902_0019_activity_customer_company.sql`: 일정에 고객사 칸 `activity.customer_company_id`
  ...
  않습니다. 고객사에 연락처가 있는 딜은 그 사람을 연결하고(직함 있는 사람 우선, 그중 가장 먼저
  등록된 사람), 일정은 붙어 있는 딜을 따라갑니다. **삭제가 있습니다.** 고객사에 연락처가 0명인
  딜(`DEMO-SEED-002`~`010`)과 딜도 고객사도 없는 `[지시] …` 할일 일정은 연결할 사람 자체가
  없어 지웁니다. `[지시]` 일정에 물린 보고서는 원본이 사라지면 근거를 잃으므로 함께 지웁니다.
  `deleted_at`이 찍힌 행도 같이 다룹니다 — NOT NULL 은 소프트 삭제와 무관하게 전체 행에
  걸립니다. 지울 딜에 자료·보고서·발주·불만이 걸려 있거나 지울 보고서에 확정 스냅샷·첨부가
  있으면 조용히 지우지 않고 실패합니다. `customer_contact_id` 자체의 필수화는 여기서 하지
  않습니다(#110). **DB 적용이 백엔드 배포보다 먼저입니다** — 새 코드는 조회에서
  `customer_company_id`를 읽으므로 컬럼이 없으면 일정 조회가 통째로 깨집니다. 반대로 이전
...
- `20260907_0023_document_soft_delete.sql`: `document`에 `deleted_at`과 살아 있는 행만 담는
  부분 인덱스를 더합니다. 자료실 자료 삭제(팀장 전용)를 행 삭제가 아니라 `deleted_at` 표시로
  처리하기 위한 것입니다. `file`이 `ON DELETE` 옵션 없이 이 표를 참조해 실제 DELETE는 외래키에
  막히고, 파일 행을 먼저 지우면 OCR·요약 결과와 감사 기록(`document_file_audit`)이 함께
  사라집니다. 기존 행은 전부 `NULL`(살아 있음)이라 백필이 없습니다. **DB 적용이 백엔드 배포보다
  먼저입니다** — 새 코드는 목록·상세 조회에서 `deleted_at`을 읽습니다.
  **2026-09-07 적용했습니다. 지금은 개발과 운영이 같은 DB라 이 적용이 곧 운영 적용입니다.**

- `20260907_0024_member_badge_color.sql`: `member`에 `badge_color`(`#rrggbb` 소문자, CHECK)를
  더하고, 개발 DB에 있는 표본 담당자 넷(`scripts/seed_sample_bracelet.py`의 `OWNER_EMAILS`)에만
  은은한 기본값을 채웁니다. 팀장이 팀 관리에서 고른 색을 담당자 이름표(`OwnerName`)에 그대로
  칠해 일정·딜·일일보고 목록에서 담당자를 색으로 가르기 위한 것입니다. `NULL`은 미지정이라
  화면이 지금까지의 회색을 쓰므로, 색을 정하지 않은 사람과 새로 들어온 사람은 그대로 보입니다.
