...
> 기준 문서: `docs/technical/SalesLuv_ERD.md`

## 1. 데이터 수집 목적

- 고객·미팅·계약·일정 이력을 연결해 영업 활동을 관리한다.
- 미팅 자료에서 C/S 사항을 추출하고 계약 정보와 함께 딜 승산 점수를 산출한다.
- 미팅 원문과 분석 결과를 바탕으로 보고서 초안을 작성하고 사용자 승인 후 계약관리 흐름에 전달한다.
...
2절 수집 범위를 기준으로 원천 참고자료 확보 현황을 정리한다. 모든 파일은 `data-sanitization-rules.md` 비식별화 규칙을 적용한 변형본이다.

| 필요 자료 (2절 기준) | 확보 현황 | 보유 파일 |
| --- | --- | --- |
| 상품·가격 정보 | `확보` | `data/sample/★MEDICAL_DEVICE_PRICE_LIST_2020.7.15기준_게시용_데모.xlsx` |
| 매출·계약 이력 | `확보` | `data/sample/3개회사_매출데이터_2022-2024_샘플완료.xlsx`, `data/sample/Sales_DB.xlsx` |
| 발주 | `확보` | `data/sample/국내대리점 발주서양식 20170426.xlsx` |
| 계약 결제 상태(선납·미납) | `확보` | `data/sample/선납_미납서.xls` |
| 고객사 검증 정보(사업자등록증) | `확보` | `data/sanitized_docs/(주)온케어메딕 사업자등록증_2017.04.19.pdf` |
| 결제 이력 | `확보` | `data/sanitized_docs/(주)한빛펫_24시새봄동물의료센터_카드결제_20241118.docx` |
| 제품 인허가 정보 | `확보` | `data/sanitized_docs/FM-Clot 수입업허가증.pdf`, `data/sanitized_docs/FM-Clot 품목허가증.pdf` |
| 고객 담당자·연락처 원부 | `미확보` | 원천자료 없음 — 3.2 서비스 운영 데이터로 대체 예정 |
| 실제 미팅 원문(녹음·메모) | `미확보` | 원천자료 없음 — 3.2 서비스 운영 데이터로 누적 예정 |
| C/S 문의·응대 이력 | `미확보` | 원천자료 없음 — 3.2 서비스 운영 데이터로 누적 예정 |

- 확보한 자료의 실제 출처, 수집 건수 및 라이선스: `확인 필요`
- 미확보 항목은 원천 문서를 별도로 구하지 않고, 서비스 운영 중 누적되는 데이터로 채운다.

### 3.2 서비스 운영 데이터

영업 담당자가 서비스에서 직접 입력한 고객·미팅·계약·일정과 Agent 분석 결과를 누적한다. 미팅 분석 Agent는 녹음·STT, 미팅 메모 또는 직접 입력한 자료를 분석하고, 보고서 작성 Agent는 원문과 분석 결과를 사용해 초안을 생성한다.
...
- 사용 목적: 미팅 분석 Agent의 딜 승산 점수(0~100) 산출 모델 학습·검증
...
- 전처리 후 유효 건수: 448건 중 완전 중복 83건 제거 후 365건
- 클래스 분포: `Won` 173건, `Lost` 192건(비율이 비슷해 SMOTE·증강·class weight 미적용)
- 분할 방식: 고정 train/test 분할 대신 10개 입력 특성 조합을 그룹으로 지정한 `StratifiedGroupKFold`(5-Fold × 시드 변경 10회 반복, 총 50개 평가 fold)
- 모델 성능: `OneHotEncoder + LogisticRegression(C=0.1, L2)` 기준 ROC-AUC 0.793 · Brier Score 0.190 · Accuracy 71.0%(검증 기준일 2026-08-17, 상세는 `딜_승산_점수_데이터_전처리_및_AI_학습_모델.md` 참고)

딜 승산 점수는 0~100점으로 산출해 화면에 표시하며 사용자 승인 대상이 아니다. 보고서 초안은 미팅 원문, 미팅 분석 결과와 C/S 사항을 함께 사용하고 사용자 승인 후 완성한다.
...
- [ ] `B2B_Sales_data` 리비전, 취득일, 체크섬 기록(클래스 분포·전처리 후 건수·모델 성능은 4.2절에 반영 완료)
